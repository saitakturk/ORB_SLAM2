...
{
#ifdef OnlineLoop
    std::cout << "Online semi-dense mapping" << std::endl;
#else
    std::cout << "Offline semi-dense mapping" << std::endl;
#endif
...
#pragma omp parallel for schedule(static) num_threads(25)  collapse(2)
...
#pragma omp parallel for schedule(static,20) num_threads(25)  collapse(2)
...
void ProbabilityMapping::UpdateModel(ORB_SLAM2::KeyFrame* kf)
{
   //unique_lock<mutex> lock(mMutexTranscript);
     mpModeler->AddLineSegmentKeyFrameEntry(kf);
     mpModeler->AddTexture(kf);
    if (mpModeler->CheckNewTranscriptEntry()) {
    //mpModeler->RunOnce();
    mpModeler->RunRemainder();
