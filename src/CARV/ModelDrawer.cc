...
#include <cstdio>
    ...
    void ModelDrawer::DrawModel(bool bRGB)
    {
        ...
        //cout << imAndTexFrame[0].first << endl;
        if (imAndTexFrame.size() >= numKFs) {
            ...
            auto keyframe_id = (imAndTexFrame[0].second).id;
            ofstream outfile;
            //write texture model to text file
            string filename = "ObjectFiles/carv/meshes/texture_coord.txt";
            outfile.open(filename.c_str());
            outfile << "mtllib texture" << keyframe_id << ".mtl" << endl;
            outfile <<"usemtl material" << keyframe_id << std::endl;
...
//                        glBindTexture(GL_TEXTURE_2D, frameTex[indexCurr]);
                        ...
                        //std::cout << index1 << " " << index2 << " " << index3 << endl;
                        //if(indices_set.find(index1) == indices_set.end())
                        outfile << "v " <<  point0[0] << " " << point0[1] << " " << point0[2] << std::endl;
                        outfile << "v " << point1[0] << " " << point1[1] << " " << point1[2] << std::endl;
                        outfile << "v " <<   point2[0] << " " << point2[1] << " " << point2[2] << std::endl;
                        outfile << "vn " << normal(0) << " " << normal(1) << " " << normal(2) << std::endl;
                        outfile << "vt " << 1- uv0[0] << " " << 1- uv0[1] << std::endl;
                        outfile << "vt " << 1- uv1[0] << " " << 1- uv1[1] << std::endl;
                        outfile << "vt " <<1-  uv2[0] << " " << 1- uv2[1] << std::endl;
                        outfile << "f " << (3 * count_faces + 1) << "/" << (3 * count_faces + 1) << "/" << count_faces + 1 << " " 
                                << (3 * count_faces + 2) << "/" << (3 * count_faces + 2) << "/" << count_faces + 1 << " " 
                                << (3 * count_faces + 3) << "/" << (3 * count_faces + 3) << "/" << count_faces + 1 << std::endl; 
                
                        ++count_faces;
                        indices_set.insert(index1);
                        indices_set.insert(index2);
                        indices_set.insert(index3);
                        break;
                    }
                }
                }
            ...
            outfile.close();
            //put lock
            string lock_file_name = "ObjectFiles/lock";
            bool ok = static_cast<bool>(std::ofstream(lock_file_name).put('l'));
            boost::filesystem::copy_file(filename, "ObjectFiles/carv/meshes/texture.obj", boost::filesystem::copy_option::overwrite_if_exists);
            //remove lock
